...
int  sockefd;
#define PORT 9090
#define SERVER_IP "172.16.8.168"
//文件映射地址
char *mmapfd = NULL;
//创建文件
#define FILEDIR "../file"
#define FILESIZE 250*1024*1024
...
void SendFileHeadInfo(const char* FileName,int Filesize)
{
    ...
    char *buf = malloc(sizeof(type)+sizeof(FileHeadInfo));
...
void CrateFile(void)
{
    ...
    mmapfd = mmap(NULL, filestat.st_size, PROT_WRITE|PROT_READ, MAP_SHARED, fd, 0);
    if (mmapfd == NULL){
        perror("mmap");
