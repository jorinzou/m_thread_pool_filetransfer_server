...
struct thInfo *TraverseThPool(void)
{
    ...
    for(i = 0; i < thpool->ThreadNum;i++){
        ...
        if (thpool->thinfo[i].Iswork == 0){
            return &(thpool->thinfo[i]);
        }
    }
    return NULL;
}
...
void ReceiveParse(int ClinetFd)
{
    ...
    if (SocketRecord->pFileHeadInfo != NULL){
       if (SocketRecord->ReceFlag == 0){
            SocketRecord->IsDataComing = 1;
            struct thInfo* thinfo = TraverseThPool();
            if (thinfo != NULL){
              pthread_cond_signal(&(thinfo->cond)); 
              return;
            }
            else{
               DEBUG_INFO("all thread is working");
              return;
            }
       }
       else{
